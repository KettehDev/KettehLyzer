...
$suspiciousPatterns = @(
    ...
    "SafeAnchor", "AirAnchor", "AnchorAction", "AnchorMacro",
    
    # Auto features
    "AutoTotem", "autototem", "auto totem", "InventoryTotem", "HoverTotem", "legittotem",
    "AutoPot", "autopot", "auto pot", "speedPotSlot", "strengthPotSlot", "AutoPotRefill",
    "AutoArmor", "autoarmor", "auto armor", "AutoDoubleHand", "autodoublehand",
    "AutoClicker", "AutoMace", "MaceSwap", "SpearSwap", "StunSlam",
    ...
    "PrestigeClient", "prestigeclient.vip", "Prestige",
    "Asteria", "AsteriaClient", "CatleanClient", "catlean",
    "Xenon", "XenonClient", "gypsy", "GypsyClient",
    "VirginClient", "ArgonClient", "Pandaware", "Skilled", "MoonClient",
    
    # Bypass
    "GrimBypass", "VulcanBypass", "MatrixBypass", "AACBypass",
    "VerusDisabler", "IntaveBypass", "WatchdogBypass",
    ...
    "ＡｕｔｏＰｏｔ", "Ａｕｔｏ Ｐｏｔ", "Ａｕｔｏ Ｐｏｔ Ｒｅｆｉｌｌ",
    "ＡｕｔｏＡｒｍｏｒ", "Ａｕｔｏ Ａｒｍｏｒ", "ＡｕｔｏＤｏｕｂｌｅＨａｎｄ",
...
function Invoke-ObfuscationScan {
    ...
    try {
        ...
        if ($fwPct    -gt  0) { $flags.Add("Fullwidth Unicode class names — $fwPct% use ａｂｃ/ＡＢＣ/０１２ chars") }
        if ($jpPct    -gt  0) { $flags.Add("Japanese obfuscation — $jpPct% use hiragana/katakana class names") }
...
foreach ($s in $suspiciousPatterns) { [void]$cheatStringSet.Add($s) }
